...
sleep(1);
require '../config.php';
$sql = "SELECT * FROM trainings INNER JOIN woredas INNER JOIN categories ON woreda = woredas.id AND category = cid WHERE woredas.wid = $woreda";
if (isset($_POST['request'])) {   
    $request = $_POST['request'];
    $sql = "SELECT * FROM trainings INNER JOIN woredas INNER JOIN categories ON woreda = woredas.id AND category = cid WHERE woredas.wid = $woreda AND year = $request";
}
